...
| App launcher icon (`android/app/src/main/res/mipmap-*`, `ios/Runner/Assets.xcassets`, `web/icons`) | Raster (PNG), baked from original code | OS home screen | Original — baked by `tool/art/bake_app_icon.py` (a golden horse at full gallop racing up the board's gold track toward a radiant open Book, a crescent above, on emerald; regenerate with that script, which writes the iOS set without an alpha channel) | Proprietary (project) | No |

## App icon

The launcher icon is an original emblem, the game in one glance: a
golden horse at full gallop racing up the board's gold track toward the
light of an open Book, a crescent in the sky above — the race for
Islamic knowledge (Iqra, then Quest) on an emerald ground. No depiction
of any person, no Kaaba-as-object, no text (spec §23 + religious
constraints). It is
