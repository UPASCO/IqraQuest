...
| App launcher icon (`android/app/src/main/res/mipmap-*`, `ios/Runner/Assets.xcassets`, `web/icons`) | Raster (PNG), baked from `tool/art/source/app_icon_source.webp` | OS home screen | Owner-supplied artwork (three horses racing across the board's tiles toward an open book with a glowing question mark, crescent and star above, gold frame); prepared and resized by `tool/art/bake_app_icon.py`, which crops the painted frame away so the platform mask frames the art, sharpens the small sizes and writes the iOS set without an alpha channel | Proprietary (project) | No |
...
hand-edited: `tool/art/bake_app_icon.py` crops the painted gold frame
away (the platform's squircle or circle is the frame; a second one
inside it wastes a tenth of the icon and gets clipped at the corners),
blends the source's black corners to the ground as a safety net, gives
the sizes at or below 120 px a light unsharp mask, and writes every
Android, iOS and web size, the iOS set without an alpha channel (App
Store icons must not carry one). A 1024px review copy and a
